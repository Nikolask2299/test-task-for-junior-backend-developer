...
func main() {
	...
	recRepo := postgresrepo.NewRecurrenceRepository(pool)
	recUsecase := recurrence.NewRecurrenceService(recRepo, taskRepo)
	recHandler := httphandlers.NewRecurrenceHandler(recUsecase)

	router = transporthttp.NewRecurrenceRoute(router, recHandler)
	...
	shedurepo := postgresrepo.NewShedulerRepository(pool)

	sheduler := sheduler.NewSheduler(ctx, taskRepo, recRepo, shedurepo)
	...
	go func() {
		if err := sheduler.Start(); err != nil {
			logger.Error("run sheduler", "error", err)
			os.Exit(1)
		}
	}()
